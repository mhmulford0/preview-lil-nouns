...
contract LilNounsOracle is OpenZeppelinAccess.Ownable {
     ...
     * @param expectedBlock is the largest block number for which the transaction is
     * considered valid. If the block.number is greater than expectedBlock when the
     * transaction is processed, it will be immediately reverted, and the Oracle
     * will not attempt to start the auction.
     */
    function settleCurrentAndCreateNewAuction(uint256 expectedBlock)
        external
        payable
    {
        require(
